...
页面默认进入“论文展示版”，首屏只保留答辩时最需要讲清楚的内容：
...
为了避免页面像调试后台一样臃肿，下面这些内容默认折叠，点击后再展开：

- 论文展示摘要：系统流程、当前结果概览和论文流程覆盖情况。
- 异常协议与数据说明：synthetic anomaly injection、Individual / Group / Registration 协议边界。
- 高级控制与状态：热力透明度、时间窗口、事件阈值、播放速度、序列统计和媒体状态。
- 方法流程、子模块和时间线：方法处理流程、分数分解、事件级证据。
- 导出与报告包：当前视图 JSON、排行榜 CSV、序列 JSON、画面 PNG 和完整报告包。

如果需要检查算法接入状态、数据流审计、key policy、运行来源和数据集质量，可以在顶部把“展示模式”切换为“完整审计版”。审计版会显示 `算法接入` 和 `数据流审计` 等完整技术标签页。
...
其中“播放 / 暂停”“帧滑块”“四画面对比 / 单画面模式”放在主控制条中；热力透明度、时间窗口、事件阈值、播放速度和序列状态被收进“高级控制与状态”，需要调参或排查时再展开。
...
“论文展示版”默认只显示前三类结果，保证答辩时先讲实验结论；“算法接入”和“数据流审计”会在切换到“完整审计版”后显示。

这些模块用于回答“当前系统接入了哪些结果、哪些数据完整、哪些方法只是 proxy 或 baseline”的问题。

页面还提供轻量导出功能。为了减少首屏按钮数量，导出入口默认收在“导出与报告包”折叠面板中：
