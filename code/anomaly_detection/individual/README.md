...
- 预留群体异常检测输入窗口和分数输出接口。
- 统一的 fused track pipeline：从同一份 `observations_<split>.csv` 生成 individual trajectories、fused trajectories、group windows、summary 和 manifest。
...
│   ├── observation_standardization.py # RGB/Thermal/modal 观测标准化
│   ├── individual_trajectories.py    # 单目标轨迹视图
│   ├── fused_track_pipeline.py       # 跨模态/跨帧轨迹统一构建入口
...
├── export_fused_track_pipeline.py
...
### 2. 导出统一 fused track pipeline 产物

如果后续实验或系统展示需要同时使用个体轨迹、融合轨迹和群体窗口，优先使用统一 pipeline：
...
```text
outputs/vt_tiny_mot_fused_pipeline_train/
├── individual_trajectories_train.jsonl
├── fused_trajectories_train.jsonl
├── group_windows_train.jsonl
├── fused_track_pipeline_summary_train.json
└── fused_track_pipeline_manifest_train.json
```

其中：

- `individual_trajectories_<split>.jsonl`：按 `sample_id = "{sequence}:{track_id}"` 聚合后的跨帧 object trajectory。
