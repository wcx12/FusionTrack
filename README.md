...
- 当前已完成一版严格对齐的验证集实验：个体级每个方法 829 条 score，群体级每个方法 15605 条 score。
- 最终汇总结果保存在本地 `server_artifacts/final_results_20260521/`。该目录用于保存服务器/本地实验产物，默认不提交到 GitHub。
...
说明：`fusiontrack_group_graph` 已经是我们的群体级方法，但它目前是非学习型图关系/规则打分方法，当前验证集结果弱于强经典 baseline。后续如果要把群体级作为论文亮点，建议继续实现学习型 group model。
...
结果来自 `server_artifacts/final_results_20260521/experiment_summary.md`，日期为 2026-05-21。官方 GPU 实验使用 NVIDIA GeForce RTX 5090。指标包括 AUROC、AUPRC、F1、P@100 和 R@100。
...
| `official_dcdetector` | Official paper baseline | Learning | 0.517620 | 0.104025 | 0.182018 | 0.420000 | 0.506024 |
| `official_anomaly_transformer` | Official paper baseline | Learning | 0.493314 | 0.106029 | 0.183223 | 0.130000 | 0.156627 |
...
| `official_dcdetector` | Official paper baseline | Learning | 0.505261 | 0.005743 | 0.012371 | 0.060000 | 0.068182 |
...
- 当前群体级 FusionTrack 方法是非学习型 `fusiontrack_group_graph`，已经实现但不是最终强模型；后续应补充学习型 group model，并和 `group_prediction_linear`、官方 TSAD baseline 公平对比。
