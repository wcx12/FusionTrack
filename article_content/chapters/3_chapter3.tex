...
The single-stage pipeline provides a compact baseline for understanding the later design. In contrast, the two-stage pipeline introduces an additional refinement step, which motivates the distinction between coarse reference retrieval and subsequent target-level association in this thesis.
...
Table~\ref{tab:tfvpr-main-results-a} covers the conventional VPR benchmarks and several long-term or seasonal datasets. It shows the main pattern of the training-free design: the proposed descriptor does not always match supervised descriptors on the easiest splits, but it improves substantially over direct frozen-token pooling. The second part of the table is discussed separately because the SVOX condition splits are easier to read when they are not compressed into the same paragraph of analysis.
